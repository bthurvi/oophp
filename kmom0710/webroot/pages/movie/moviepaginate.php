...
function getHitsPerPage($hits) {
  ...
  foreach($hits AS $val) {
    $nav .= "<a class='aButton' href='" . htmlspecialchars(getQueryString(array('hits' => $val))) . "'>$val</a> ";
  }  
  return $nav;
}
...
{
  $nav  = "<a class='aButton' href='" . htmlspecialchars(getQueryString(array('page' => $min))) . "'>&lt;&lt;</a> ";
  $nav .= "<a class='aButton' href='" . htmlspecialchars(getQueryString(array('page' => ($page > $min ? $page - 1 : $min) ))) . "'>&lt;</a> ";

  for($i=$min; $i<=$max; $i++) {
    $nav .= "<a class='aButton' href='" . htmlspecialchars(getQueryString(array('page' => $i))) . "'>$i</a> ";
  }

  $nav .= "<a class='aButton' href='" . htmlspecialchars(getQueryString(array('page' => ($page < $max ? $page + 1 : $max) ))) . "'>&gt;</a> ";
  $nav .= "<a class='aButton' href='" . htmlspecialchars(getQueryString(array('page' => $max))) . "'>&gt;&gt;</a> ";
  return $nav;
}
...
<table class='table'>
  ...
  {
    echo "<tr><td style='width:10%;'>$i</td><td style='width:10%;'>{$row->id}</td>"
    . "<td style='width:15%;'><img src='{$row->image}' alt='bild på film' width='100' height='50'></td>"
    . "<td>{$row->title}</td><td style='width:10%;'>{$row->year}</td></tr>";
  }
?>
</table>
